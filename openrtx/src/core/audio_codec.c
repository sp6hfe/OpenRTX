...
static const uint16_t filtAlpha = 655;  // 1e-2 * 65536
static const uint16_t openTsh = 6;      // equiv. to -40dB
static const uint16_t closeTsh = 0;
...
static void *encodeFunc(void *arg)
{
    ...
    while(reqStop == false)
    {
        ...
        if(!dsp_pwrSquelchEvaluate(&sqlState, openTsh, closeTsh))
